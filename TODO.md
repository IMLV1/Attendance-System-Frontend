...
| **บั๊ก #4 `/profile` ฟิลด์ว่าง** | ✅ **ไม่ใช่บั๊ก** — `system-root` ไม่มีข้อมูลในฐานตั้งแต่ต้น (`fullname_thai`/`gender`/`phone` ว่างจริง) บัญชีปกติแสดงครบทุกฟิลด์ |
...
- [ ] **`/statistic` KPI 4 ตัวรวมเป็นแถวเดียว** — งานใหญ่สุดที่เหลือ ตอนนี้ 2 ตัวอยู่ใน
      `WorkingDay` อีก 2 ตัวอยู่ใน `WorkingHour` คนละ widget **คนละ request**
      ต้องดึงออกมาเป็น stat tile กลาง + เผื่อเคสข้อมูลมาไม่พร้อมกัน
