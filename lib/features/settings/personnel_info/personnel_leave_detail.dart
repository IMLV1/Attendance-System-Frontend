...
class PersonnelLeaveDetail extends StatefulWidget {
  final String requestID;
  final int permissionLevel;
  final void Function() onApproved;
  final void Function() onRejected;

  const PersonnelLeaveDetail({
    super.key,
  ...
  @override
  State<PersonnelLeaveDetail> createState() {
    return _PersonnelLeaveDetailState();
  }
}

class _PersonnelLeaveDetailState extends State<PersonnelLeaveDetail> {
                                          ...
                                          onPressed: () async {
                                            if (setting!.approveNeedSignature) {
                                              ...
                                              await provider.push(context, ServiceSignaturePage(
                                                ...
                                                request: (pngByte) => LeaveApprovalService().approval(widget.requestID, 'rejected', _textEditingController.text, pngByte),
                                          ...
                                          onPressed: () async {
                                            if (setting!.approveNeedSignature) {
                                              ...
                                              await provider.push(context, ServiceSignaturePage(
                                                ...
                                                request: (pngByte) => LeaveApprovalService().approval(widget.requestID, 'approved', _textEditingController.text, pngByte),
                                                onSuccessResponse: (pngBytes, jsonData) {
                                                  navigator.pop();
                                                  widget.onApproved();
